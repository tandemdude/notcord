...
export default function SignUpForm({setShowSignUp}) {
    ...
    const formik = useFormik({
        ...
        validate: values => {
            ...
            if (values.password != null && values.passwordConf != null) {
            ...
            if (values.username != null && /^[\w\-.]{5,40}$/.test(values.username)) {
    ...
    return (
        <form className="sign-up-form space-y-6 relative" onSubmit={formik.handleSubmit}>
            ...
            <div>
                ...
                {usernameValid ? null :
                    <p className="text-rose-400 text-xs">Username is invalid. May only contain: a-z, A-Z, 0-9,
                        _, -, .</p>}
            </div>
            <div>
            ...
            <div>
                ...
                <input id="passwordConf" name="passwordConf" type="password" onChange={formik.handleChange}
                       ...
                       className="border text-sm rounded-lg block w-full p-2.5 bg-gray-600 border-gray-500 placeholder-gray-400 text-white"/>
                {passwordsMatch ? null :
                    <p id="pass-conf-alert" className="text-rose-400 text-xs">Passwords do not match</p>}
            </div>
            {loading ? <SubmitButtonLoading/> : <SubmitButtonInteractible content={"Sign Up"}/>}
            <div className="text-sm font-medium text-gray-300">
                Already registered? <a onClick={() => setShowSignUp(false)}
